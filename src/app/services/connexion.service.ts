    ...
    return this.http.post<LoginResponse>(`${this.apiUrl}/login`, credentials, { withCredentials: true }).pipe(
      tap(response => {
        ...
        if (response.status) {
          ...
          if (role === 'utilisateur') {
            this.router.navigate(['/dashutilisateur']); // Redirection vers le dashboard utilisateur
          } else if (role === 'Super Admin') {
            this.router.navigate(['/dashboard']); // Redirection vers le dashboard admin
          } else {
            console.error('⚠️ Rôle inconnu:', role);
          }
        } else {
